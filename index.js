...
didUseBannedWords = text => {
  return (
    bannedSubstrings.some(bannedSubstring => {
...
handleMention = event => {
  let postMessage = text => {
    web.chat
      .postMessage({ channel: event.channel, text: text })
      .catch(console.error);
  };
  ...
  switch (command) {
    ...
    case "taco":
      postMessage(
        "@ a user in the same message as the tacos you'd like to give them. You can give up to 10 in a day. Include the reason for the gift in your message"
      );
      break;
    case "help":
      postMessage(
        "@ me with one of the keywords: status, fortune, scoreboard, taco, or help"
      );
      break;
    case "help me":
    ...
    .then(scoreboardData => {
      ...
      scoreboardData.forEach(userScores => {
        tableData.push([
          userScores.name,
          userScores.bans,
          userScores.bannedTime,
          userScores.totalTacosGiven,
          userScores.totalTacosReceived
        ]);
      });
      let responseString =
        "```" +
      ...
      web.chat
        .postMessage({ channel: event.channel, text: responseString })
        .catch(console.error);
    })
    .catch(console.error);
};
      ...
      .then(responses => {
        ...
        allUserScores.forEach(userScores => {
          scoreboardData.push({
            ...
            bannedTime: helpers
              .secondsToString(userScores.bannedSeconds)
              .trim(),
...
slackEvents.on("message", event => {
  if (event.user === "UBP9JBB2B") return;
  ...
  if (event.channel != process.env.IGNORE_CHANNEL) {
        ...
        .then(() => {
          web.users
            ...
            .then(response => {
              ...
              offenseService
                .getOffensesForUserInLast24Hours(event.user)
                .then(userOffenses => {
                  ...
                  if (doesMentionBot(event.text)) {
                    web.chat
                      .postMessage({
                        ...
                        text: `${user.profile.display_name ||
                          user.real_name} insulted my mother and is therefore kicked for 24 hours.`
                      })
                      .then(() => {
                        ...
                        offenseService.createOffense(
                          event.user,
                          twentyFourHoursInSeconds
                        );
                        scoresService.updateBanScore(
                          event.user,
                          twentyFourHoursInSeconds
                        );
                      })
                    ...
                    web.chat
                      .postMessage({
                        channel: event.channel,
                        text: `${user.profile.display_name ||
                          user.real_name} was kicked for using a banned phrase. This is their ${helpers.ordinalOf(
                          offenseNumber
                        )} offense in the last 24 hours. They will be reinvited after ${
                          offenseTime.words
                        }.`
                      })
                      .then(() => {
    ...
    if (event.channel === "DBN4H8DQT") {
      // if a direct message from slackbot
    ...
    if (event.text && event.text.toLowerCase().includes("apparently")) {
      ...
      if (Math.random() > 0.75) {
        web.chat
          .postMessage({
            channel: event.channel,
            text: gifLinks[Math.floor(Math.random() * gifLinks.length)]
          })
          .catch(console.error);
      }
    }
    ...
    if (
      ...
      const tacoRecipientId = event.text
        .split("<")
        .pop()
        .split(">")
        .shift()
        .replace("@", "")
        .trim();
      tacoTransactionService
        .getNumberOfGiftedTacoTransactionsByUserInLastDay(event.user)
          ...
          if (event.text.split(">").length > 2) {
            web.chat
              .postMessage({
                channel: event.channel,
                text:
                  "You can only give tacos to one person at a time (for now)"
              })
              .catch(console.error);
            ...
            web.chat
              .postMessage({
                channel: event.channel,
                text: "You can only give up to 10 tacos a day"
              })
              .catch(console.error);
            ...
            web.chat
              .postMessage({
                ...
                text: `<@${
                  event.user
                }> you can only give 10 tacos in a day. You have ${10 -
                  giftedTacos} left to give today.`
              })
              .catch(console.error);
        ...
        .then(() => {
          web.users
            ...
            .then(response => {
              let user = response.user;
              offenseService
                .getOffensesForUserInLast24Hours(userWhoKickedMe)
                .then(userOffenses => {
                  const offenseNumber = userOffenses.length + 1;
                  let offenseTime = getOffenseTime(offenseNumber);
                  web.chat
                    .postMessage({
                      channel: event.channel,
                      text: `${user.profile.display_name ||
                        user.real_name} was kicked for kicking me. This is their ${helpers.ordinalOf(
                        offenseNumber
                      )} offense in the last 24 hours. They will be reinvited after ${
                        offenseTime.words
                      }.`
...
slackEvents.on("reaction_added", event => {
  if (
    event.reaction !== "kick" ||
    event.item.channel === process.env.IGNORE_CHANNEL
  ) {
    return;
  }
  ...
  web.reactions
    ...
    .then(response => {
          ...
          .then(() => {
            web.users
              ...
              .then(response => {
                let user = response.user;
                offenseService
                  .getOffensesForUserInLast24Hours(userId)
                  .then(userOffenses => {
                    const offenseNumber = userOffenses.length + 1;
                    let offenseTime = getOffenseTime(offenseNumber);
                    web.chat
                      .postMessage({
                        channel: channelId,
                        text: `${user.profile.display_name ||
                          user.real_name} voted to kick me, so I kicked them. This is their ${helpers.ordinalOf(
                          offenseNumber
                        )} offense in the last 24 hours. They will be reinvited after ${
                          offenseTime.words
                        }.`
      ...
      let hasBeenKickedForMessageAlready =
        voteKickedMessages.includes(event.item.ts) ||
        voteKickedMessages.includes(event.item.file) ||
        voteKickedMessages.includes(event.item.file_comment);
      if (
        kickReaction &&
        kickReaction.count >= 100 &&
        !hasBeenKickedForMessageAlready
      ) {
        voteKickedMessages.push(
          event.item.ts || event.item.file || event.item.file_comment
        );
        web.groups
          .kick({
            channel: channelId,
              ...
              .then(response => {
                ...
                offenseService
                  ...
                  .then(userOffenses => {
                    const offenseNumber = userOffenses.length + 1;
                    let offenseTime = getOffenseTime(offenseNumber);
                    let usersVoteList = `<@${kickReaction.users[0]}>, <@${
                      kickReaction.users[1]
                    }>, <@${kickReaction.users[2]}>, and <@${
                      kickReaction.users[3]
                    }>`;
                    web.chat
                      .postMessage({
                        channel: channelId,
                        text: `${usersVoteList} have spoken. ${user.profile
                          .display_name ||
                          user.real_name} has been removed from the chat. This is their ${helpers.ordinalOf(
                          offenseNumber
                        )} offense in the last 24 hours. They will be reinvited after ${
                          offenseTime.words
                        }.`
